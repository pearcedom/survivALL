...
We begin with an expressionSet `nki_subset`, which details both gene expression as well as complete survival information (i.e. event and time-to-event data with no missing data) for 319 invasive breast cancer patients of no specific subtype. Here the event we will be measuring is distant metastasis free survival or *dmfs*.  
...
p_med <- GGally::ggsurv(median_fit, surv.col = c("#525252", "#bdbdbd")) +
    ylim(0.4, 1) +
    labs(title = "Figure S1", subtitle = "Median approach", x = "Years", fill = "Class") +
    theme_pander() + 
    theme(axis.line = element_line(size = 0.1), 
          legend.position = 'bottom')
l_km <- get_legend(p_med)
p_med <- p_med + theme(legend.position = 'none')
p_med <- ggdraw(add_sub(p_med, "p = 0.9", vpadding=grid::unit(0, "lines"), y = 17, x = 0.55, hjust = 0, size = 10))
p_med
```
...
However, knowing that ERBB2 is overexpressed in ~25% of invasive breast cancer cases, we can modify our cohort stratification to relfect this.
...
p_hyp <- GGally::ggsurv(hypothesis_fit, surv.col = c("#525252", "#bdbdbd")) +
    ylim(0.4, 1) +
    labs(title = "Figure S2", subtitle = "Hypothesis-driven approach", x = "Years", fill = "Class") +
    theme_pander() + 
    theme(axis.line = element_line(size = 0.1), 
          legend.position = 'none')
p_hyp <- ggdraw(add_sub(p_hyp, "p = 0.043", vpadding=grid::unit(0, "lines"), y = 17, x = 0.55, hjust = 0, size = 10))
p_hyp
```
...
p_data <- GGally::ggsurv(data_fit, surv.col = c("#525252", "#bdbdbd")) +
    ylim(0.4, 1) +
    labs(title = "Figure S4", subtitle = "Data-drive approach", x = "Years", fill = "Class") +
    theme_pander() + 
    theme(axis.line = element_line(size = 0.1), 
          legend.position = 'none')
p_data <- ggdraw(add_sub(p_data, "p = 0.001", vpadding=grid::unit(0, "lines"), y = 17, x = 0.55, hjust = 0, size = 10))
p_data
```
